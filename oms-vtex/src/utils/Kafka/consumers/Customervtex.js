...
async function handleVtexOrderMessage(message, ctx) {
  ...
  // 1) Obtener VTEX (con reintentos/timeout)
  let vtexData = null;
  try {
    vtexData = await fetchVtexOrder(orderId, { retries: 4, timeout: 15000 });
  } catch (e) {
    console.warn(` VTEX fetch fallo para ${orderId}: ${e.message}`);
  }
  ...
  // Evitar "órdenes en blanco": si es creación y VTEX viene vacío, no postear al OMS
  const vtexLooksEmpty =
    !vtexData ||
    !Array.isArray(vtexData.items) || vtexData.items.length === 0 ||
    !Number.isFinite(Number(vtexData.value));

  if (created && vtexLooksEmpty) {
    await setOrderErrorIntegration(orderPkId, 'VTEX_DATA_NOT_FOUND_OR_EMPTY');
    console.warn(' OMS POST omitido: vtexData vacío o sin items', { orderPkId, commerceId: orderId });
    return;
  }
  ...
  try {
    ...
    // Validación mínima de payload antes de postear
    const payloadLooksEmpty =
      !payload ||
      !Array.isArray(payload.items) || payload.items.length === 0 ||
      !Number.isFinite(Number(payload.doctotalsy));

    if (payloadLooksEmpty) {
      await setOrderErrorIntegration(orderPkId, 'OMS_PAYLOAD_INVALID: EMPTY_OR_NO_ITEMS');
      console.warn(' OMS POST omitido por payload vacío', { orderPkId, commerceId: orderId });
      return;
    }
    ...
    // ✅ tratar duplicados como éxito idempotente
    if (status === 409 || /ORDER_EXISTS/i.test(String(data?.message || data || code || ''))) {
      await setOrderErrorIntegration(orderPkId, 'ORDER_EXISTS');
      const existingId = data?.id ?? null;
      if (existingId) await updateOrderWithOmsId(orderPkId, existingId);
      console.warn(' OMS: ORDER_EXISTS (catch)', { orderPkId, commerceId: orderId, id: existingId });
      return;
    }
