...
const PickingService = {
  ...
  updatePickedProduct: async (orderProductID, pickedQuantity) => {
    ...
    console.log(
      `✅ Producto ${orderProductID} actualizado con ${pickedQuantity} unidades recogidas.`
    );
  ...
  completePicking: async (orderID) => {
    ...
    const hasMissingProducts = await PickingRepository.hasMissingProducts(
      orderID
    );
    if (hasMissingProducts) {
      await sendMessage("order.status.updated", { orderID, newStatus: 4 });
      console.log(
        `📤 Estado de la orden ${orderID} -> 4 (Picking Completado Parcialmente)`
      );
    } else {
