...
const crearUsuario = async (req, res) => {
  ...
  if (!Array.isArray(rolesIds) || rolesIds.some(id => typeof id !== 'number')) {
    return res.status(400).json({ error: 'El campo "rolesIds" debe ser un arreglo de números.' });
  }
  ...
  // Validaciones suaves de canal (SOLO si vienen)
  if (canalDeVenta !== undefined && canalDeVenta !== null && typeof canalDeVenta !== 'string') {
    return res.status(400).json({ error: 'El campo "canalDeVenta" debe ser string si se proporciona.' });
  }
  if (canalDeVentaId !== undefined && canalDeVentaId !== null && typeof canalDeVentaId !== 'string') {
    return res.status(400).json({ error: 'El campo "canalDeVentaId" debe ser string si se proporciona.' });
  }
  ...
  try {
    ...
    const perfil = {
      ...
      telefono,
      urlImagenPerfil,
      canalDeVenta: canalDeVenta ?? null,
      canalDeVentaId: canalDeVentaId ?? null
    };
...
const editarUsuario = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: 'Usuario y perfil actualizados correctamente.' });
  } catch (error) {
    console.error('Error al actualizar usuario:', error);
    const num = error?.number || error?.originalError?.info?.number;
    if (num === 2601 || num === 2627) {
